...
When run immediately after 'Glassbox debug', the active session is used instead.

Examples:
  Glassbox explain 5c0a1234567890abcdef1234567890abcdef1234567890abcdef1234567890ab
  Glassbox explain --network testnet <tx-hash>
  Glassbox debug <tx-hash> && Glassbox explain`,
...
func explainFromSession() error {
	...
	if sess == nil {
		return fmt.Errorf("no active session; run 'Glassbox debug <tx-hash>' first or provide a transaction hash")
	}
...
func init() {
	...
	explainCmd.Flags().StringVar(&explainRPCToken, "rpc-token", "", "RPC authentication token (can also use GLASSBOX_RPC_TOKEN env var)")
