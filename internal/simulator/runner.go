...
import (
	...
	"github.com/dotandev/glassbox/internal/bridge"
...
func findSimBinary(simPathOverride string) (string, string, error) {
	...
	if env := os.Getenv("GLASSBOX_SIM_PATH"); env != "" {
		if isExecutable(env) {
			return abs(env), "env GLASSBOX_SIM_PATH", nil
		}
	}
	...
	if err == nil {
		localCandidates := []string{
			filepath.Join(cwd, "glassbox-sim"),
			filepath.Join(cwd, "bin", "glassbox-sim"),
		}
	...
	devCandidates := []string{
		filepath.Join("simulator", "target", "debug", "glassbox-sim"),
		filepath.Join("simulator", "target", "release", "glassbox-sim"),
	}
	...
	// 5. Global PATH
	if p, err := exec.LookPath("glassbox-sim"); err == nil {
		return p, "global PATH", nil
	}

	return "", "", errors.WrapSimulatorNotFound("glassbox-sim binary not found (use --sim-path or set GLASSBOX_SIM_PATH)")
}
...
// request field if set, otherwise env GLASSBOX_SANDBOX_NATIVE_TOKEN_CAP_STROOPS.
func getSandboxNativeTokenCap(req *SimulationRequest) *uint64 {
	...
	if v := os.Getenv("GLASSBOX_SANDBOX_NATIVE_TOKEN_CAP_STROOPS"); v != "" {
...
// request field if set, otherwise env GLASSBOX_SIM_MEMORY_LIMIT_BYTES.
func getSimulatorMemoryLimit(req *SimulationRequest) *uint64 {
	if req != nil && req.MemoryLimit != nil {
		return req.MemoryLimit
	}
	if v := os.Getenv("GLASSBOX_SIM_MEMORY_LIMIT_BYTES"); v != "" {
		if n, err := strconv.ParseUint(v, 10, 64); err == nil {
			return &n
		}
	}
...
func getSimulatorCoverageLCOVPath(req *SimulationRequest) *string {
	...
	if v := os.Getenv("GLASSBOX_SIM_COVERAGE_LCOV_PATH"); v != "" {
		return &v
	}
	return nil
}
...
// to match GLASSBOX_LOG_LEVEL so both the Go and Rust sides honour the same level.
func simulatorEnv() []string {
	env := os.Environ()

	erstLevel := os.Getenv("GLASSBOX_LOG_LEVEL")
	if erstLevel == "" {
		return env
	}
